...
export async function run(provider: NetworkProvider) {
    ...
    const fastnet_proper_key_blocks_seqnos = await get_recent_proper_key_blocks(fastnet_ls_pair);

    // Deploying lite-client
    const lite_client_contract = provider.open(await LiteClientContract.createFromConfig(fastnet_ls_pair, fastnet_proper_key_blocks_seqnos[0]));

    await lite_client_contract.sendDeployLiteClient(provider.sender(), testnet_ls_pair, fastnet_proper_key_blocks_seqnos[0] , toNano('0.1'));
